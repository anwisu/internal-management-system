...
function EmptyState({ message = 'No data available', actionLabel, onAction, icon: Icon }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-white/50 backdrop-blur-sm border border-blue-gray-100 rounded-3xl shadow-sm" role="status" aria-live="polite">
      <div className="w-16 h-16 mb-4 rounded-full bg-blue-gray-50 flex items-center justify-center text-blue-gray-300 shadow-inner">
        {Icon ? <Icon className="w-8 h-8" aria-hidden="true" /> : <FiInbox className="w-8 h-8" aria-hidden="true" />}
      </div>
      <p className="text-lg font-medium text-blue-gray-700 mb-1">
        {message}
      </p>
      <p className="text-sm text-blue-gray-500 mb-6 max-w-sm">
